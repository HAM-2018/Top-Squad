...
export async function getTeamChallengeProgressDaily(): Promise<TeamProgress | null> {
  ...
  const teams = teamChallenges.map((t) => ({
    teamId: t.teamId,
    name: t.teamName,
    isMyTeam: t.teamId === challenge.myTeamId,
  }));
  ...
  // date bounds from attempts
  if (teamChallengeIds.length === 0) {
    return { challengeId: challenge.challengeId, teams, overall: [], parts: {} };
  }
  ...
  for (const row of perUserBest) {
    ...
    if (isTeamLogOnly) {
      ...
      // for non-team-log-only:
      // time = average of best per user
      // others = sum of best per user
      totals.set(teamId, (totals.get(teamId) ?? 0) + v);
      counts.set(teamId, (counts.get(teamId) ?? 0) + 1);
    }
  }

  // if time and not teamLogOnly, average it
  ...
  // values for ALL teams (null if no attempt yet)
  const values: Record<number, number | null> = {};
  for (const teamId of teamIds) {
    values[teamId] = totals.has(teamId) ? (totals.get(teamId) ?? null) : null;
  }

  // ranks only for teams with values
  ...
  // for overall points (sum of ranks), we need ranksByPart
  const ranksByPart = new Map<number, Record<number, number>>();

  for (const p of parts) {
    const { values, ranks } = await valuesAndRanksForPartAtDay(
      p.id,
      p.metric,
      p.isTeamLogOnly,
      t
    ...
    // store per-part VALUES history
    const key = String(p.id);
    if (!partsOut[key]) partsOut[key] = [];
    partsOut[key].push({ t, values });

    // store ranks for overall
    ranksByPart.set(p.id, ranks);
  }

  // overall points = sum of ranks across parts (lower = better)
  ...
  // convert to values object for chart (points per team)
  const overallValues: Record<number, number | null> = {};
  for (const teamId of teamIds) {
    overallValues[teamId] = pointsByTeam.has(teamId)
      ? pointsByTeam.get(teamId)!
      : null;
  }

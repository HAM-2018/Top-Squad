...
export default function LogScoreCard({
    ...
    const selectedChallenge = useMemo(
        () => challenges.find((c) => String(c.teamChallengeId) === selectedTcId) ?? null,
        [challenges, selectedTcId]
    );
     const [selectedPartId, setSelectedPartId] = useState<string>("");
     const selectedPart = useMemo(
        () => selectedChallenge?.parts.find((p) => String(p.partId) === selectedPartId) ?? null,
    ...
    async function onSubmit() {
        ...
        toast.error("Error", { description: e?.message ?? "Failed to record score" });
        }
    }

    return (
        <Card className="relative w-full max-w-5xl">
        <CardHeader>
            <CardTitle>Record a Challenge score</CardTitle>
        </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="grid gap-1">
                    ...
                    <SelectTrigger className="h-10">
                        <SelectValue placeholder={mode === "solo" ? "Select solo challenge" : "Select team challenge"} />
                    </SelectTrigger>

                    <SelectContent>
                        {challenges.map((c) => (
                        <SelectItem key={c.teamChallengeId} value={String(c.teamChallengeId)}>
